...
class OPEN_VIDEO_TRACKER_OT_run_pipeline_modal(bpy.types.Operator):
    ...
    def modal(self, context, event):
        if event.type == 'TIMER':
            ...
            if self._thread and not self._thread.is_alive():
                # Thread has finished
                if self._process.returncode == 0 or not self._current_step < 6:
                    if self.model_dir and os.path.exists(self.model_dir):
                        import_colmap_data(context , self.model_dir , self.image_dir)
    ...
    def processs(self, context):
        ...
        self.report({'INFO'}, "Step 1/6: Extracting frames...")
        self.update_current_step(1)
        OPEN_VIDEO_TRACKER_OT_run_pipeline_modal._message = "Step 1/6: Extracting frames..."
        ...
        try:
            ...
            self.report({'INFO'}, "Step 2/6: Extracting features...")
            self.update_current_step(2)
            print("Step 2/6: Extracting features...")
            OPEN_VIDEO_TRACKER_OT_run_pipeline_modal._message = "Step 2/6: Extracting features..."
            cmd = [
                prefs.colmap_path,
                "feature_extractor",
            ...
            self.report({'INFO'}, "Step 3/6: Matching features...")
            self.update_current_step(3)
            print("Step 3/6: Matching features...")
            OPEN_VIDEO_TRACKER_OT_run_pipeline_modal._message = "Step 3/6: Matching features..."
            ...
            print("Step 4/6: Running sparse reconstruction...")
            OPEN_VIDEO_TRACKER_OT_run_pipeline_modal._message = "Step 4/6: Running sparse reconstruction..."
            cmd = [
                prefs.glomap_path,
                "mapper",
                "--database_path", database_path,
            ...
            # Step 5: Export TXT to parent sparse directory
            self.report({'INFO'}, "Step 5/6: Exporting model (external)...")
            self.update_current_step(5)
            print("Step 6/: Exporting model (external)...")
            OPEN_VIDEO_TRACKER_OT_run_pipeline_modal._message = "Step 6/6: Exporting model (external)..."
            if os.path.exists(model_dir):
                ...
                self.report({'WARNING'}, "No model found to export")
                return  # Early exit if model doesn't exist
        ...
        self.report({'INFO'}, "Step 7: Importing model...")
        print("Step 7: Importing model...")
        ...
        OPEN_VIDEO_TRACKER_OT_run_pipeline_modal._message = "Step 7: Importing model..."
