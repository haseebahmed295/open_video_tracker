...
from .object_utility import (
    add_collection,
    add_obj,
)

from .object_utility import add_empty
...
def draw_points(
    ...
    log_info("Add particle draw handlers", op)
...
def compute_principal_point_shift(camera, relativ_to_largest_extend):
    ...
    # print('INFO', 'shift_x: ' + str(shift_x), op)
    # print('INFO', 'shift_y: ' + str(shift_y), op)
...
def add_cameras(
    ...
    if add_image_planes:
        log_info("Adding image planes: True", op)
        ...
        log_info("Adding image planes: False", op)

    if add_depth_maps_as_point_cloud:
        log_info("Adding depth maps as point cloud: True", op)
        ...
        log_info("Adding depth maps as point cloud: False", op)
    ...
    for index, camera in enumerate(cameras):
        ...
        if not os.path.isfile(image_path):
            log_warning("Could not find image at " + str(image_path), op)
            continue
        else:
            log_info("Found image at " + str(image_path), op)
    ...
    log_info("Duration: " + str(stop_watch.get_elapsed_time()), op)
    log_info("Adding Cameras: Done", op)
...
def add_camera_image_plane(
    ...
    # print('INFO', 'add_camera_image_plane: ...', op)
    # print('INFO', 'name: ' + str(name), op)
    ...
    mesh.update()
    mesh.validate()
    # print('INFO', 'add_camera_image_plane: Done', op)
    return mesh_obj
